 ...
 * props:
 * - open (bool)
 * - asset (object) with fields: id, name, purchase_date, cost, depreciation_method, useful_life, category, description
 * - onClose (fn)
 * - onUpdate(updatedAsset) (fn)
 */
function EditAssetModal({ open, asset, onClose, onUpdate }) {
  const { token } = useAuth();
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await axios.put(`/api/assets/${asset.id}`, form, {
  ...
  return (
    <>
      ...
      >
        <Alert severity="success">Asset updated successfully!</Alert>
      </Snackbar>
    </>
  );
}
